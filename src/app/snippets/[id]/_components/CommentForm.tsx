...
function CommentForm({ onSubmit, isSubmitting }: CommentFormProps) {
  ...
  const headerCls1 = "bg-blue-500/10 text-blue-400";
  const headerCls2 = "hover:bg-[#ffffff08] text-gray-400";

  const placeholder = `Add to the discussion... ${`\n`}${"```"}javascript${`\n`}let hey = [];${`\n`}${"```"}`
    ...
    <form onSubmit={handleSubmit} className="mb-8">
      <div className="bg-[#0a0a0f] rounded-xl border border-[#ffffff0a] overflow-hidden">
        ...
        <div className="flex justify-end gap-2 px-4 pt-2">
          ...
          className={`text-sm px-3 py-1 rounded-md transition-colors ${isPreview ? headerCls1 : headerCls2}`} >
        ...
        {/* Comment form body */}
        {
          ...
          onChange={(e) => setComment(e.target.value)} onKeyDown={handleKeyDown} placeholder={placeholder}
