...
function ThemeSelector() {
  ...
  return (
    <div className="relative" ref={dropDownRef}>
      ...
      className="w-48 group relative flex items-center justify-between gap-2 px-4 py-2.5 bg-[#1e1e2e]/80 hover:bg-[#262637] 
      rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700" >
        ...
        <span className="text-gray-300 min-w-[80px] text-center group-hover:text-white transition-colors">
          {currentTheme?.label}
        </span>
      ...
      <AnimatePresence>
        {
          isOpen && (
            <motion.div className="absolute top-full left-0 mt-2 w-full min-w-[240px] bg-[#1e1e2e]/95 
            ...
            {
              THEMES.map((t, index) => (
                <motion.button className={`relative group w-full flex items-center gap-3 px-3 py-2.5 hover:bg-[#262637]
